...
export async function SoftDeleteAction({
  ...
  try {
    if (tableName === "programs") {
      ...
      const { data } = await supabase
        .from("travel_order")
        .select("travel_order_no, user_id")
        .eq("id", recordId)
        .maybeSingle();
      ...
      activityCode = "Deleted a Record";
      activityDescription = `${tableName.replaceAll("_", " ")} deleted.`;
    }
  } catch (contextError) {
    console.error(
      "Failed to gather delete notification context:",
      contextError,
    );
    notificationMessage = `${tableName.replaceAll("_", " ")} deleted.`;
    activityCode = "Deleted a Record";
